...
import static org.cytoscape.opencl.cycl.CyCLUtils.*;
...
//      CyCLUtils.checkCLError(CL10.clBuildProgram(program, device.getDevice(), buildOptions.toString(), null, NULL));
  ...
  public String getProgramBuildInfoStringASCII(long cl_device_id, int param_name) {
    try (MemoryStack stack = stackPush()) {
      PointerBuffer pp = stack.mallocPointer(1);
      checkCLError(clGetProgramBuildInfo(program, cl_device_id, param_name, (ByteBuffer)null, pp));
      int bytes = (int)pp.get(0);

      ByteBuffer buffer = stack.malloc(bytes);
      checkCLError(clGetProgramBuildInfo(program, cl_device_id, param_name, buffer, null));
